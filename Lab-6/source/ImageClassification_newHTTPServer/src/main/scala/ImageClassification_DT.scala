...
object ImageClassification_DT {
  //val dt_matrix=new MulticlassMetrics()
  var cm=""
  var acc=""
  ...
  def main(args: Array[String]) {
    ...
    val server = HttpServer.create(new InetSocketAddress("192.168.1.146",8080), 0)
    server.createContext("/get_dt", new RootHandler())
    ...
    class RootHandler extends HttpHandler {
      def handle(httpExchange: HttpExchange) {
        ...
        val response="Image Classification Decision Tree\n\nAccuracy:"+acc+"\n\nConfusion matrix:\n"+cm
